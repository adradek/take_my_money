...
RSpec.describe ShoppingCartsController do
  describe "PATCH #update" do
    ...
    let(:performance) { instance_spy(Performance, event: build_stubbed(:event)) }
    let(:workflow) { instance_spy(AddsToCart) }

    before do
      allow(controller).to receive(:current_user).and_return(user)
      allow(Performance).to receive(:find).with("2").and_return(performance)
      allow(AddsToCart).to receive(:new).with(user: user, performance: performance, count: "1").and_return(workflow)
    end

    context "when successful" do
      it "adds tickets to a shopping cart", :aggregate_failures do
    ...
    context "when unsuccessful" do
      it "redirects back to the event", :aggregate_failures do
